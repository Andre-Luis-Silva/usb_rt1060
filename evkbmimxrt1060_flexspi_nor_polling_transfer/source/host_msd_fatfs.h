...
typedef enum
{
   ...
   DATA_PACKAGE,
   CS_PACKAGE
} t_protocolSavingFirmwareSteps;

typedef struct
{
  uint8_t initialByte;
  uint8_t quantBytes[2];
  uint8_t addrBytes[4];
  uint8_t initialAddrBytes[4];
  uint8_t fullAddrBytes[8];
  uint8_t commandBytes[2];
  uint8_t dataBytes[32];
  uint8_t checkSumBytes[2];
} protocol_hex_firmware_t;
